...
@Component({
  ...
  templateUrl: './cherry-supply-reports.component.html',
  styleUrls: ['./cherry-supply-reports.component.css']
})
export class CherrySupplyReportsComponent implements OnInit {

  title = 'Cherry supply report';
  ...
  constructor(private formBuilder: FormBuilder,
              ...
              private locationService: LocationService, private cherrySupplyService: CherrySupplyService) {
  }
  ...
  onSubmit(searchBy: string) {
    if (this.filterForm.valid) {
      ...
      this.cherrySupplyService.report(filters).subscribe((data) => {
        this.loading = false;
        this.total = 0;
        if (data.content.length !== 0) {
          this.reportData = [];
          data.content.map((item) => {
            const temp = [item.name, item.totalCherries];
            this.total = this.total + item.totalCherries;
